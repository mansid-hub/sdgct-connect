      ...
      {slides.map((slide, i) => (
        <div
          ...
          className={cn(
            ...
            active === i ? "opacity-80" : "opacity-20"
          )}
        ...
        <div className="max-w-3xl">
          {slides.map((slide, i) => (
            <div key={i} className={cn(active === i ? "block" : "hidden")}>
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-7xl font-bold leading-[1.05] text-balance animate-fade-up" style={{ animationDelay: "80ms" }}>
                {slide.title}
              </h1>
              <p className="mt-6 text-base sm:text-lg lg:text-xl text-primary-foreground/85 max-w-2xl leading-relaxed animate-fade-up" style={{ animationDelay: "180ms" }}>
                {slide.description}
              </p>
            </div>
          ))}
            ...
            {slides.map((_, i) => (
              <button
                ...
                className={cn(
                  ...
                  active === i ? "w-10 bg-accent" : "w-6 bg-primary-foreground/40 hover:bg-primary-foreground/60"
                )}
              />
            ))}
          </div>
          <div className="hidden sm:flex items-center gap-3 text-xs uppercase tracking-widest text-primary-foreground/70">
